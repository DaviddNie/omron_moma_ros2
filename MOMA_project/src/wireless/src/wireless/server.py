...
class WirelessServer(Node):
    ...
    def __init__(self):
        ...
        self.joint_config_map = {
            ...
            "camera_home": [-90.0, -11.09, 89.32, 0.0, 90.0, 0.0],
    ...
    ##       Helper Functions
    ###########################################
    async def call_service_and_return_ok(self, client, request):
    ...
    def handle_wireless_info(self, request, response):
        try:
            result = self.run_async(self.handle_wireless_info_async(request))
    ...
    async def handle_wireless_info_async(self, request):
        commands = {
            LIGHT_COMMAND: lambda: self.light_request(request.param1),
            LISTENNING_COMMAND: lambda: self.listenning_request(request.param1),
            MOVE2JOINTPOS: lambda: self.move2JointConfig(request.param1),
            MOVE2POINT_COMMAND: lambda: self.move2Point(request.param1),
            GRIP_COMMAND: lambda: self.gripper_engage(True),
            RELEASE_COMMAND: lambda: self.gripper_engage(False),
            EXIT_COMMAND: self.send_exit
        }
    ...
    async def gripper_engage(self, engage):
        ...
        return await self.call_service_and_return_ok(self.cli_3, req)
    ...
    async def light_request(self, param1):
        ...
        req.value = param1
        return await self.call_service_and_return_ok(self.cli, req)
    ...
    async def move2JointConfig(self, param1):
        ...
        req.fine_goal = False
        return await self.call_service_and_return_ok(self.cli_2, req)
...
def main(args=None):
    ...
    # wireless_server = WirelessServer()
    # rclpy.spin(wireless_server)
    # #wireless_server.light_request()
    # #Shutdown after the request
    # wireless_server.destroy_node()
    # rclpy.shutdown()
